...
function Vocab:__init(path, crop)
  self.size = 0
  self._index = {}
  self._tokens = {}
  ...
  while true do
    ...
    if line == nil or (crop and self.size == crop) then break end
    self.size = self.size + 1
    local w = line:split('\t')[1] -- File might have second col with counts
  ...
  local roots = {'<t>', '<T>'}
  for _, tok in pairs(roots) do
    self.root_index = self.root_index or self._index[tok]
    if self.root_index ~= nil then
      self.root_token = tok
      break
    end
  end
...
function Vocab:reverse_map(indices,kill_nil)
  ...
  for k,v in pairs(indices) do
    local tok  = self:token(v)
    if not kill_nil then
      output[k] = tok
    elseif tok then
      total_valid = total_valid + 1
      output[total_valid] = tok
    end
  end
  return output
end
...
function Vocab:add_root_token()
  if self.root_token ~= nil then return end
  self.root_index = self:add('<t>')
  self.root_token = '<t>'
end
